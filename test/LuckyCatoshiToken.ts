...
describe("Lucky Catoshi ERC20", function () {
  async function deployFixture() {
    const [owner, addr1, addr2, pairAddr, liquidity, dev, market, aridrop] =
  ...
  describe("Deployment", function () {
    it("Should have correct initial configuration", async function () {
      const { catoshi, decimals, owner, liquidity, dev, market } =
  ...
  describe("Transfer", function () {
    it("Should owner transfer tokens", async function () {
      const { catoshi, owner, liquidity, addr1, addr2 } = await loadFixture(
        deployFixture
      );

      await catoshi.connect(liquidity).transfer(addr1.address, 1000);
      expect(await catoshi.balanceOf(addr1.address)).to.equal(1000);

      const liqBalance = await catoshi.balanceOf(liquidity.address);
      expect(liqBalance).to.equal("499999999999999999999999000");
    });

    it("Should transfer tokens after launch", async function () {
      const { catoshi, liquidity, addr1, addr2, pairAddr } = await loadFixture(
        deployFixture
      );
    ...
    it("Should fail if not launch", async function () {
      const { catoshi, liquidity, addr1, addr2 } = await loadFixture(
        deployFixture
      );
      await catoshi.connect(liquidity).transfer(addr1.address, 1000);
  ...
  describe("Burn", function () {
    it("Should burn tokens", async function () {
      const { catoshi, addr1, liquidity, decimals } = await loadFixture(
        deployFixture
      );

      await catoshi
        .connect(liquidity)
        .transfer(addr1.address, parseUnits("1", decimals));
      await catoshi.setLaunch();

      await catoshi.connect(liquidity).burn(parseUnits("1", decimals));
